...
blockman.fire = blockman.fire or {}
local fire = blockman.fire
blockman.expose_global("fire", fire)

local S = minetest.get_translator("blockman")

minetest.register_alias("fire:basic_flame", "blocklife:basic_flame")
minetest.register_alias("fire:permanent_flame", "blocklife:permanent_flame")
minetest.register_alias("fire:flint_and_steel", "blocklife:flint_and_steel")
...
local function flood_flame(pos, _, newnode)
	...
	if minetest.get_item_group(newnode.name, "igniter") == 0 then
		minetest.sound_play("blocklife_fire_extinguish_flame",
...
local fire_node = {
	drawtype = "firelike",
	tiles = {{
		name = "blocklife_fire_basic_flame_animated.png",
		animation = {
	...
	inventory_image = "blocklife_fire_basic_flame.png",
	paramtype = "light",
	light_source = 13,
	walkable = false,
	buildable_to = true,
...
minetest.register_node("blocklife:basic_flame", flame_fire_node)

-- Permanent flame node
local permanent_fire_node = table.copy(fire_node)
permanent_fire_node.description = S("Permanent Fire")

minetest.register_node("blocklife:permanent_flame", permanent_fire_node)

-- Flint and Steel
minetest.register_tool("blocklife:flint_and_steel", {
	description = S("Flint and Steel"),
	inventory_image = "blocklife_fire_flint_steel.png",
	sound = {breaks = "blocklife_default_tool_breaks"},

	on_use = function(itemstack, user, pointed_thing)
		local sound_pos = pointed_thing.above or user:get_pos()
		minetest.sound_play("blocklife_fire_flint_and_steel",
		...
		if pointed_thing.type == "node" then
			...
			elseif minetest.get_item_group(node_under, "flammable") >= 1
				...
				minetest.set_node(pointed_thing.above, {name = "blocklife:basic_flame"})
			end
		end
...
minetest.register_craft({
	output = "blocklife:flint_and_steel",
	recipe = {
		{"blocklife:flint", "blocklife:steel_ingot"}
	}
})

-- Override coalblock to enable permanent flame above
-- Coalblock is non-flammable to avoid unwanted basic_flame nodes
minetest.override_item("blocklife:coalblock", {
	after_destruct = function(pos)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "blocklife:permanent_flame" then
			minetest.remove_node(pos)
		end
	end,
	on_ignite = function(pos)
		...
		if minetest.get_node(flame_pos).name == "air" then
			minetest.set_node(flame_pos, {name = "blocklife:permanent_flame"})
		end
	end
})
...
if flame_sound then
	...
	function fire.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"blocklife:basic_flame", "blocklife:permanent_flame"}
		)
		-- Total number of flames in radius
		local flames = (num["blocklife:basic_flame"] or 0) +
			(num["blocklife:permanent_flame"] or 0)
		-- Stop previous sound
		...
		if flames > 0 then
			...
			-- Play sound
			local handle = minetest.sound_play("blocklife_fire_fire", {
				pos = fposmid,
...
if fire_enabled then
	...
	minetest.register_abm({
		...
		action = function(pos)
			...
			if p then
				minetest.set_node(p, {name = "blocklife:basic_flame"})
			end
		end
	})
	...
	minetest.register_abm({
		...
		nodenames = {"blocklife:basic_flame"},
