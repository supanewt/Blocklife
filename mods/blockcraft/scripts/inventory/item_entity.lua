...
local item = {
	...
	burn_up = function(self)
		...
		self.object:remove()
		minetest.sound_play("blocklife_default_item_smoke", {
			pos = p,
			gain = 1.0,
			max_hear_distance = 8,
		}, true)
	...
	on_step = function(self, dtime, ...)
		...
		if self._pickup_delay then
			self._pickup_delay = self._pickup_delay - dtime
			if self._pickup_delay <= 0 then
				self._pickup_delay = nil
			end
		end

		if not self._pickup_delay and self.itemstring and self.itemstring ~= "" then
			local pos = self.object:get_pos()
			if pos then
				local stack = ItemStack(self.itemstring)
				for _, player in ipairs(minetest.get_connected_players()) do
					local ppos = player:get_pos()
					if ppos and vector.distance(ppos, pos) <= 1.3 then
						...
						if inv and inv:room_for_item("main", stack) then
							...
							self.itemstring = leftover:to_string()
							self.object:set_properties({infotext = leftover:get_description()})
							return
						end
					end
				end
